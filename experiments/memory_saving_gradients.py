...
def gradients_memory(ys, xs, grad_ys=None, **kwargs):
    ##################
    logging.WARN("aaaaaaaaa for memory!!")
    ##################
...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
    ...
    bwd_ops = ge.get_backward_walk_ops([y.op for y in ys],
                                       inclusive=True)
    ##################
    logging.info("2222222222222222222222 in our gradient!!")
    ##################
    ...
    for x in checkpoints:
        if x.op and x.op.name is not None:
            # calculate derivate for x means that x will not be freed and will be a reference for grad. node
    ...
    # partial derivatives to the checkpointed tensors and xs, backward ops created from seed_ops (i.e., the loss)
    ...
    # add checkpoint as the source of grad_node
    ...
    # Algo.2: incorporate derivatives flowing through the checkpointed nodes
