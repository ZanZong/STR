...
from common.definitions import remat_data_dir
from common.graph_plotting import render_dfgraph
from common.load_keras_model import MODEL_NAMES, get_keras_model, CHAIN_GRAPH_MODELS
from common.profile.cost_model import CostModel
from common.profile.platforms import PLATFORM_CHOICES, platform_memory, pretty_platform_name
from common.ray_utils import get_futures
...
from remat.core.solvers.strategy_hybrid_ilp import solve_hybrid_ilp
from remat.tensorflow2.extraction import dfgraph_from_keras

np.set_printoptions(threshold=np.inf)
# ILP solve params
NUM_ILP_CORES = os.environ.get("ILP_CORES", 32 if os.cpu_count() > 32 else 4)
...
PLOT_UNIT_RAM = 1024 * 1024 * 1024  # = GB
...
def extract_params():
    ...
    parser.add_argument('--skip-baseline', action='store_true', help="If set, skip running the ILP(checkmate) during evaluation.")
...
def log_strategy(solve_strategy, results, log_file, g):
    baseline_cpu = np.sum(list(g.cost_cpu.values()))
    # Scatter candidate solutions
    valid_data = [(r.schedule_aux_data, r.solve_time_s) for r in results if r is not None and r.schedule_aux_data is not None]
    sorted_data = sorted(valid_data, key=lambda r: r[0].peak_ram)
    data_points = [(t[0].peak_ram / PLOT_UNIT_RAM, t[0].cpu * 1.0 / baseline_cpu, t[1]) for t in sorted_data]
    with open(log_file, "a+") as wlog:
        wlog.write(f"{solve_strategy}, {data_points}\n")
...
if __name__ == "__main__":
    ...
    if args.platform == "flops":
        ...
        # Profiled data is storaged in aws s3 by authors.
    ...
    if not args.skip_ilp:
        ...
        if len(args.ilp_eval_points) > 0:
            ...
            global_eval_points = get_global_eval_points(g, result_dict) # for feature maps memory only
            print(f"Evaluating ILP at global evaluation points: {global_eval_points}")
            futures = []
            for b in global_eval_points:
                seed_result = get_closest_budget_result(result_dict, b) # baseline result which closes to `b` as the search seed
            ...
            # using current minimal-memory solution to generate the budget value
        ...
        # run local search routine, search from the current memory budget.
        # This step may find the fesible solution with lower memory cost, but it's not our target.
        if not args.skip_baseline:
            logger.info("Running local search through checkmate with budget: {0}".format(local_ilp_eval_points))
            futures = []
        ...
        log_strategy(SolveStrategy.OPTIMAL_ILP_GC, result_dict[SolveStrategy.OPTIMAL_ILP_GC], exp_log, g)

        # optimize mixed problem
        futures = []
        for b in local_ilp_eval_points:
            seed_result = get_closest_budget_result(result_dict, b)
            seed_s = seed_result.schedule_aux_data.S if seed_result is not None else None
            future = remote_ilp_mixed(g, b, time_limit=args.ilp_time_limit, solver_cores=NUM_ILP_CORES, seed_s=seed_s,
                                write_log_file=ilp_log_base / f"ilp_mixed_{b}.log", print_to_console=False,
                                write_model_file=ilp_log_base / f"ilp_mixed_{b}.lp" if args.debug else None,
                                eps_noise=0 if args.exact_ilp_solve else 0.01, approx=args.exact_ilp_solve)
            futures.append(future)
        result_dict[SolveStrategy.MIXED_ILP_OPTIMAL].extend(get_futures(futures, desc="Local mixed optimal ILP sweep"))
        log_strategy(SolveStrategy.MIXED_ILP_OPTIMAL, result_dict[SolveStrategy.MIXED_ILP_OPTIMAL], exp_log, g)

    logger.info("Saving result to {0}.".format(log_base))
    ####
    # Plot result_dict
    ####
    ...
    export_prefix_min = {}
    logger.info(f"\nPlaning for given memory budget {local_ilp_eval_points}")
    ...
    for solve_strategy, results in result_dict.items():
        ...
        sorted_data = sorted(valid_data, key=lambda r: r[0].peak_ram)
        data_points = [(t[0].peak_ram / PLOT_UNIT_RAM, t[0].cpu * 1.0 / baseline_cpu, t[1]) for t in sorted_data]
        logger.info(f"Strategy {solve_strategy} has {len(data_points)} results: \n {data_points}")
        if solve_strategy in [SolveStrategy.MIXED_ILP_OPTIMAL, SolveStrategy.OPTIMAL_ILP_GC] and len(results) == 1:
            logger.info(results[0].schedule_aux_data.mem_timeline)
