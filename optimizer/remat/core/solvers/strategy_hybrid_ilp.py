...
from remat.core.utils.solver_common import SOLVER_DTYPE, SolverTarget
from remat.core.utils.scheduler import schedule_from_rspq
from remat.core.utils.result_check import *
...
class HybridILPSolver:
    ...
    def solve(self):
        ...
        Pout = np.zeros((T, T), dtype=remat.core.utils.solver_common.SOLVER_DTYPE if self.integral else np.float)
        Qout = np.zeros((T, T), dtype=remat.core.utils.solver_common.SOLVER_DTYPE if self.integral else np.float)
...
def solve_hybrid_ilp(g: DFGraph, budget: int, seed_s: Optional[np.ndarray] = None, approx=True,
    ...
    try:
        ...
        if not check_compute_correctness(g, s, r):
            ...
            from remat.core.utils.approximate_hybrid import fine_grained_approx
