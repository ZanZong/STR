...
class HybridILPSolver:
    def __init__(self, g: DFGraph, budget: int, eps_noise=None, seed_s=None, integral=True,
                 ...
                 target=SolverTarget.MIN_COMPUTE, cpu_fwd_factor=2, batch_size_min=None, idx_pq2r=None, idx_r2pq=None):
        ...
        self.idx_pq2r = idx_pq2r # <pq_index: origin_index>
        self.idx_r2pq = idx_r2pq # <origin_index: pq_index[]>
        self.TS = len(self.idx_pq2r.keys())
        ...
        # cost_cpu and ram must be expanded if idx_pq2r and idx_r2pq are not none.
        self.swap_control = SwapControler(10E9, g.cost_cpu, g.cost_ram, idx_pq2r)

        T = self.g.size
        print(f"graph size={T}, pq_mini_stages={self.TS}")
        ...
        if self.integral:
            ...
            self.P = self.m.addVars(self.TS, T, name="P", vtype=GRB.BINARY)
            self.Q = self.m.addVars(self.TS, T, name="Q", vtype=GRB.BINARY)
        else:
            # approximation
            ...
            self.P = self.m.addVars(self.TS, T, name="P", vtype=GRB.CONTINUOUS, lb=0.0, ub=1.0)
            self.Q = self.m.addVars(self.TS, T, name="Q", vtype=GRB.CONTINUOUS, lb=0.0, ub=1.0)
        ...
        with Timer("Gurobi model construction", extra_data={'T': str(T), 'budget': str(budget)}):
            ...
            with Timer("Variable initialization", extra_data={'T': str(T), 'budget': str(budget)}):
                if self.imposed_schedule == ImposedSchedule.FULL_SCHEDULE:
                    ...
                    q_zeros = []
                    for t in range(TS):
                        for i in range(self.idx_pq2r[t], T):
                            p_zeros.append(self.P[t, i])
                            q_zeros.append(self.Q[t, i])
                    self.m.addLConstr(quicksum(p_zeros), GRB.EQUAL, 0)
                    self.m.addLConstr(quicksum(q_zeros), GRB.EQUAL, 0)
                    for t in range(TS):
                        for i in range(self.idx_pq2r[t]):
            ...
            with Timer("Correctness constraints", extra_data={'T': str(T), 'budget': str(budget)}):
                ...
                for t in range(TS):
                    for i in range(T):
                        ...
                        if start_stage is not None:
                            self.m.addLConstr(self.S[self.idx_pq2r[t] + 1, i], GRB.LESS_EQUAL, self.Q[start_stage, i] + self.S[self.idx_pq2r[t], i] + self.R[self.idx_pq2r[t], i])
                        else:
                            if self.idx_pq2r[t] + 1 >= T: continue
                            self.m.addLConstr(self.S[self.idx_pq2r[t] + 1, i], GRB.LESS_EQUAL, self.S[self.idx_pq2r[t], i] + self.R[self.idx_pq2r[t], i])

            with Timer("Constraint: swapping correctness", extra_data={'T': str(T), 'budget': str(budget)}):
                # 1. only swap-out once
                for i in range(T):
                    self.m.addLConstr(quicksum(self.P[t, i] for t in range(TS)), GRB.LESS_EQUAL, 1)
                ...
                n_eta = 5
                for i in range(T):
                    self.m.addLConstr(quicksum(self.P[t, i] for t in range(TS)), GRB.LESS_EQUAL, quicksum(self.Q[t, i] for t in range(TS)))
                    self.m.addLConstr(n_eta * quicksum(self.P[t, i] for t in range(TS)), GRB.GREATER_EQUAL, quicksum(self.Q[t, i] for t in range(TS)))
                ...
                for t in range(TS):
                    for i in range(self.idx_pq2r[t]):
                        # if i >= T:
                        #     break # stop because of mini stages
                ...
                for t in range(TS):
                    for i in range(self.idx_pq2r[t]):
                        ...
                        if start_stage is not None:
                            ...
                            self.m.addLConstr(self.S[self.idx_pq2r[t], i], GRB.GREATER_EQUAL, self.P[start_stage, i])
            ...
            with Timer("Constraint: initialize memory usage (includes spurious checkpoints)",
                ...
                for t in range(TS):
                    ...
                    self.m.addLConstr(self.U[self.idx_pq2r[t], 0], GRB.EQUAL,
                                    self.BS * (self.R[self.idx_pq2r[t], 0] * permute_ram[0] + quicksum(
                                        self.S[self.idx_pq2r[t], i] * permute_ram[i] for i in range(T)) \
    ...
    def solve(self):
        ...
        Pout = np.zeros((self.TS, T), dtype=stropt.core.utils.solver_common.SOLVER_DTYPE if self.integral else np.float)
        Qout = np.zeros((self.TS, T), dtype=stropt.core.utils.solver_common.SOLVER_DTYPE if self.integral else np.float)
...
def mini_stage_spliter(g: DFGraph, mini_stage_percent:95):
    """ Use percentile to divide graph to more fine-graind stages.
        Computing time larger than this percentile will be divided by the minimal one.
    """
    costs = [g.cost_cpu[key] for key in sorted(g.cost_cpu.keys())]
    percented_ = np.percentile(costs, mini_stage_percent)
    print(f"percented for {mini_stage_percent}: {percented_}")
    split_stage = {}
    unused_swap = [0, len(g.cost_cpu)] # first and last usually is large, but with less swap benefits
    for idx, cost in enumerate(costs):
        if cost >= percented_ and idx not in unused_swap:
            split_stage[idx] = cost
    tick = np.average([c for i, c in enumerate(costs) if i not in split_stage.keys()])
    for key, value in split_stage.items():
        split_stage[key] = round(value / tick)
    print(f"Selected nodes to split: {split_stage}")
    return split_stage
...
def solve_hybrid_ilp(g: DFGraph, budget: int, seed_s: Optional[np.ndarray] = None, approx=True,
    ...
    if use_mini_stage:
        idx_r2pq = dict()
        idx_pq2r = dict()
        mini_stages = mini_stage_spliter(g, 90)
        # split matmul ops into k mini-stage
        new_g_cur = 0
        for node in g.v:
            if node not in mini_stages.keys():
                idx_pq2r[new_g_cur] = node
                new_g_cur += 1
            else:
                for i in range(mini_stages[node]):
                    idx_pq2r[new_g_cur] = node
                    new_g_cur += 1
        idx_r2pq = {}
        for k, v in idx_pq2r.items():
            if v in idx_r2pq.keys():
                idx_r2pq[v].append(k)
            else:
                idx_r2pq[v] = []
                idx_r2pq[v].append(k)
        print("pq graph size={}".format(len(idx_pq2r.keys())))
    ...
    ilpsolver = HybridILPSolver(g, budget, gurobi_params=param_dict, seed_s=seed_s,
                          ...
                          solve_r=solve_r, write_model_file=write_model_file, idx_pq2r=idx_pq2r, idx_r2pq=idx_r2pq)
    ilpsolver.build_model()
    try:
        r, s, u, free_e, p, q = ilpsolver.solve()
        
        # map ministage to origin graph
        if use_mini_stage:
            def convert_pq(mat):
                new_mat = np.zeros((g.size, g.size), dtype=np.int)
                for t in range(mat.shape[0]):
                    for i in range(mat.shape[1]):
                        if mat[t, i] == 1:
                            new_mat[idx_pq2r[t], i] = 1
                return new_mat
            q = convert_pq(q)
            p = convert_pq(p)
            
        # create a original swap controler for pruning
        sc = SwapControler(10E9, g.cost_cpu, g.cost_ram)
        pruned_Qout = prun_q_opt(sc, q, s)
    ...
    # schedule, aux_data = schedule_from_rspq(g, r, s, q)
